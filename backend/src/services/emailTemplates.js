const APP_NAME = 'Michael Kairos Labs';
...
function mentionEmail({ senderName, context, appUrl }) {
  ...
  const html = baseLayout(`
    <h2 style="margin:0 0 16px;color:#111827;font-size:18px">${safeSender} mentioned you</h2>
    <div style="margin:16px 0;padding:16px;background-color:#f9fafb;border-radius:6px;border-left:4px solid #4f46e5">
      <p style="margin:0;color:#374151;font-size:14px;line-height:1.5">${safeContext}</p>
    </div>
...
function systemNotificationEmail({ title, body, appUrl }) {
  ...
  const html = baseLayout(`
    <h2 style="margin:0 0 16px;color:#111827;font-size:18px">${safeTitle}</h2>
    <p style="margin:0 0 16px;color:#374151;font-size:14px;line-height:1.5">${safeBody}</p>
  ...
  return { html, text, subject: safeTitle };
}
