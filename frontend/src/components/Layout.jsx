...
import { LayoutDashboard, User, FolderKanban, Settings, LogOut, Menu, X, Shield, ExternalLink, Search, WifiOff, GitBranch } from 'lucide-react'
...
export default function Layout() {
  ...
  const navItems = [
    ...
    { to: '/dashboard/projects', icon: FolderKanban, label: 'Client Projects', badge: projectsBadge },
    ...
    <div className="min-h-screen bg-cloud">
      ...
      <div className="fixed top-0 left-0 right-0 h-16 bg-midnight text-white flex items-center justify-between px-4 lg:px-6 z-50 shadow-nav">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 rounded-lg hover:bg-midnight-light text-slate-300"
          ...
          <div className="hidden md:flex items-center gap-2 ml-2 pl-3 border-l border-slate-600">
            ...
            {user?.role === 'admin' && (
              <span className="px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide bg-midnight-light text-slate-300 rounded">
                Admin
              </span>
            )}
          </div>
        ...
        {/* Right: search + assistant + notifications */}
        <div className="flex items-center gap-2">
          <button
            onClick={() => setSearchOpen(true)}
            className="hidden sm:flex items-center gap-2 w-72 px-3 py-1.5 text-sm text-slate-400 bg-midnight-light rounded-lg hover:bg-slate-700 transition-colors"
          >
            <Search size={16} />
            <span>Search...</span>
          ...
          <button
            onClick={() => setSearchOpen(true)}
            className="sm:hidden p-2 rounded-lg hover:bg-midnight-light text-slate-300"
            aria-label="Search"
          >
            <Search size={20} />
          </button>
      ...
      {navOpen && (
        <>
          ...
          <div
            ref={dropdownRef}
            className="fixed top-16 left-0 z-50 w-72 m-2 bg-white border border-gray-200 shadow-xl rounded-xl"
          >
            {/* User info on mobile */}
            <div className="md:hidden px-4 pt-4 pb-3 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {user?.role === 'admin' && (
                    <span className="px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide bg-primary-100 text-primary-700 rounded">
                      Admin
                    </span>
                  )}
                </div>
            ...
            <nav className="p-2 space-y-0.5">
              ...
              <a
                href="/"
                onClick={closeNav}
                className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-slate-600 hover:bg-teal-50 hover:text-teal-700 transition-colors"
              >
