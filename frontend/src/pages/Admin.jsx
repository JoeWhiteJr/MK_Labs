...
export default function Admin() {
  ...
  useEffect(() => {
    document.title = 'Admin - MKL'
  }, [])

  useEffect(() => {
    fetchStats(); fetchUnreadCountsByType()
  ...
  return (
    <div>
      <h1 className="font-display font-bold text-2xl mb-6 text-text-primary">Admin Dashboard</h1>
      <div className="flex gap-2 border-b border-gray-200 mb-6">
        {[['dashboard', 'Dashboard', LayoutDashboard], ['team', 'Team', Users], ['projects', 'Projects', FolderKanban], ['publish', 'Publish', Globe], ['recommendations', 'Suggestions', Lightbulb], ['trash', 'Trash', Trash2], ['site-content', 'Site Content', FileEdit], ['public-team', 'Public Team', UserCircle]].map(([id, label, Icon]) => (
          <button key={id} onClick={() => setActiveTab(id)} className={`flex items-center gap-2 px-4 py-3 border-b-2 text-text-secondary ${activeTab === id ? 'border-teal-500 text-teal-600' : 'border-transparent'}`}>
            <Icon size={18} />{label}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-xl p-6 border border-gray-200">
              ...
              <div className="text-3xl font-bold text-text-primary">
                {stats ? stats.users?.total_users || 0 : <span className="inline-block h-8 w-16 bg-gray-100 rounded animate-pulse" />}
              </div>
            </div>
            <div className="bg-white rounded-xl p-6 border border-gray-200">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <select value={summaryDateRange} onChange={(e) => setSummaryDateRange(e.target.value)} className="px-3 py-1.5 rounded-lg border border-purple-200 text-sm bg-white text-text-primary focus:outline-none focus:ring-2 focus:ring-purple-300">
      ...
      {activeTab === 'team' && (
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-display font-semibold text-lg text-text-primary">Team Members</h2>
            {teamMembers.length > 0 && (
              <span className="text-sm text-text-secondary">{teamMembers.length} total</span>
            )}
          </div>
          {isLoadingTeam ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <select
                      ...
                      className="px-3 py-1.5 rounded-lg border border-gray-300 text-sm bg-white text-text-primary focus:outline-none focus:ring-2 focus:ring-teal-300 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isSuperAdmin ? (
                        <>
      ...
      {activeTab === 'projects' && (
        <div className="space-y-4">
          <h2 className="font-display font-semibold text-lg text-text-primary">Project Lead Assignment</h2>
          <p className="text-sm text-text-secondary">Assign or change the lead for each project. Only admins can perform this action.</p>
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase">Project</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase">Status</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase">Current Lead</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-text-secondary uppercase">Assign Lead</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={project.id} className="border-b border-gray-100 last:border-0">
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        className="px-3 py-1.5 rounded-lg border border-gray-300 bg-white text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-teal-300"
                      >
            ...
            {projects.length === 0 && (
              <div className="p-8 text-center text-text-secondary">No projects found.</div>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === 'publish' && (
        <div>
          <h2 className="font-display font-semibold text-lg mb-6 text-text-primary">Publish Team Projects</h2>
                ...
                <div key={project.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  ...
                  <div className="h-32 bg-gradient-to-br from-teal-100 to-teal-200 flex items-center justify-center relative">
                    {(isPublished ? pp.published_image : project.header_image) ? (
                      ...
                      <div className="text-teal-400 text-4xl font-bold opacity-30">{project.title?.charAt(0)}</div>
                    )}
                    {isPublished && (
                      ...
                      {isPublished ? (
                        <>
                          <button
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 bg-teal-50 text-teal-700 rounded-lg text-sm hover:bg-teal-100"
                          >
                            <Pencil size={14} /> Edit
                          </button>
                          <button
      ...
      >
        <p className="text-text-secondary">
          Are you sure you want to remove <strong className="text-text-primary">{showDeleteConfirm?.name}</strong> from the team?
      ...
      >
        <p className="text-text-secondary">
          Are you sure you want to unpublish <strong className="text-text-primary">{showUnpublishConfirm?.published_title}</strong>?
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="w-full px-3 py-2 rounded-lg border border-gray-300 bg-white text-text-primary focus:outline-none focus:ring-2 focus:ring-teal-300 resize-none"
              />
            </div>
            <div>
